...
that match the finalized run. Raw reviewer text is never included. The finding set is always
complete: the prior-report budget bounds findings carried into a later prompt, not updater stdin,
and the reviewer result is already bounded by the output cap, so re-capping intake would only drop
